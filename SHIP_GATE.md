...
- [ ] `[cli|mcp|desktop]` SKIP: CLI has no dangerous actions — attestation and verification are non-destructive
...
- [ ] `[cli|mcp|desktop]` SKIP: CLI uses standard .NET logging, no custom levels
- [ ] `[mcp]` SKIP: not an MCP server
...
- [ ] `[desktop]` SKIP: CLI tool, not a desktop app
